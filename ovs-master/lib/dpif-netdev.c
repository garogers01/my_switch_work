...
static void dp_netdev_input(struct dp_netdev_pmd_thread *,
                            struct dp_packet **, int cnt, int port);
                      ...
                      const struct netdev_flow_key *b)
{
    ...
    return a->hash == b->hash && !rte_memcmp(&a->mf, &b->mf, a->len);
}
...
{
    return !rte_memcmp(&key->mf, mf, key->len);
}
...
extern void dpdk_emc_cache_init();
                                ...
                                const struct miniflow *mf)
{
    ...
    if (OVS_UNLIKELY(dp_packet_rss_valid(packet))) {
...
emc_processing(struct dp_netdev_pmd_thread *pmd, struct dp_packet **packets,
               ...
               struct packet_batch batches[], size_t *n_batches, int port)
{
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (port > 0) 
        {
            pkt_metadata_init(&packets[i]->md, port);
            packets[i]->md.in_port.odp_port = port;
...
dp_netdev_input(struct dp_netdev_pmd_thread *pmd,
                struct dp_packet **packets, int cnt, int port)
{
#if !defined(__CHECKER__) && !defined(_WIN32)
    const size_t PKT_ARRAY_SIZE = cnt;
#else
    ...
    long long now = rte_rdtsc();
    size_t newcnt, n_batches, i;

    n_batches = 0;
    newcnt = emc_processing(pmd, packets, cnt, keys, batches, &n_batches, port);
    ...
    OVS_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    case OVS_ACTION_ATTR_TUNNEL_PUSH:
        if (*depth < MAX_RECIRC_DEPTH) {
            ...
            if (!err) {
                (*depth)++;
                dp_netdev_input(pmd, packets, cnt, -1);
                (*depth)--;
            } else {
