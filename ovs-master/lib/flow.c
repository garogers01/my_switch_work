...
//    const void *data = dp_packet_data(packet);
    char const *data = dp_packet_data(packet);
    ...
    if (OVS_UNLIKELY(size < sizeof(struct eth_header))) {
        ...
        miniflow_push_macs(mf, dl_dst, (void *)data);
        /* dl_type, vlan_tci. */

        vlan_tci = parse_vlan((const void **) &data, &size);
        dl_type = parse_ethertype((const void **) &data, &size);
    ...
    if (OVS_UNLIKELY(eth_type_mpls(dl_type))) {
        int count;
        const void *mpls = (void *) data;

        packet->l2_5_ofs = data - l2;
        count = parse_mpls((const void **)&data, &size);
        miniflow_push_words_32(mf, mpls_lse, mpls, count);
    }

    /* Network layer. */
    packet->l3_ofs = data - l2;

    nw_frag = 0;
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_IP))) {
        const struct ip_header *nh = (const struct ip_header *) data;
        int ip_len;
        uint16_t tot_len;
...
//        data_pull(&data, &size, ip_len);
        ...
        nh = data_pull((const void **) &data, &size, sizeof *nh);
        ...
        while (1) {
            ...
            if ((nw_proto == IPPROTO_HOPOPTS)
                ...
                if (OVS_UNLIKELY(!data_try_pull((const void **)&data, &size,
                ...
                const struct ovs_16aligned_ip6_frag *frag_hdr = (const struct ovs_16aligned_ip6_frag *)data;

                nw_proto = frag_hdr->ip6f_nxt;
                if (!data_try_pull((const void **)&data, &size, sizeof *frag_hdr)) {
                    goto out;
                }
        ...
        if (dl_type == htons(ETH_TYPE_ARP) ||
            ...
            const struct arp_eth_header *arp = (const struct arp_eth_header *)
                data_try_pull((const void **)&data, &size, ARP_ETH_HEADER_LEN);
    ...
    if (OVS_LIKELY(!(nw_frag & FLOW_NW_FRAG_LATER))) {
        if (OVS_LIKELY(nw_proto == IPPROTO_TCP)) {
            ...
            if (OVS_LIKELY(size >= SCTP_HEADER_LEN)) {
                const struct sctp_header *sctp = (const struct sctp_header *) data;
            ...
            if (OVS_LIKELY(size >= IGMP_HEADER_LEN)) {
                const struct igmp_header *igmp = (const struct igmp_header *) data;
            ...
            if (OVS_LIKELY(size >= sizeof(struct icmp6_hdr))) {
                ...
                const struct icmp6_hdr *icmp = (const struct icmp6_hdr *) data_pull((const void **)&data, &size,
                                                         sizeof *icmp);
                parse_icmpv6((const void **)&data, &size, icmp, &nd_target, arp_buf);
                if (nd_target) {
...
rte_prefetch0((void *) ((uint64_t *)packet + offsetof(struct dp_packet, source)));
if (md->port_only ==0) {
    /* Metadata. */
    if (md->tunnel.ip_dst) {
        miniflow_push_words(mf, tunnel, &md->tunnel,
...
miniflow_equal(const struct miniflow *a, const struct miniflow *b)
{
    ...
    if (OVS_LIKELY(flowmap_equal(a->map, b->map))) {
        return !rte_memcmp(ap, bp, miniflow_n_values(a) * sizeof *ap);
    } else {
        size_t idx;
...
{
    return !rte_memcmp(a, b, sizeof *a
