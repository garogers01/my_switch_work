...
static int
get_dpdkpatch_config(const struct netdev *dev_, struct smap *args)
{
    struct netdev_vport *dev = netdev_vport_cast(dev_);
...
extern void netdev_dpdk_set_peer(struct netdev *dev, const char *peer);

static int
set_dpdkpatch_config(struct netdev *dev_, const struct smap *args)
{
    struct netdev_vport *dev = netdev_vport_cast(dev_);
    const char *name = netdev_get_name(dev_);
    const char *peer;
    ...
    peer = smap_get(args, "peer");
    if (!peer) {
        VLOG_ERR("%s: dpdk patch type requires valid 'peer' argument", name);
        return EINVAL;
    }

    if (smap_count(args) > 1) {
        VLOG_ERR("%s: dpdk patch type takes only a 'peer' argument", name);
        return EINVAL;
    }

#if 0
    type = netdev_get_type(dev_);
    if(strcmp(type,"dpdkvhost") || strcmp(type,"dpdkr") || strcmp(type,"dpdk")) {
        VLOG_ERR("%s: dpdk patch type requires dpdk ports", name);
        return EINVAL;
    }
#endif
    ...
    if (!dev->peer || strcmp(dev->peer, peer)) {
        ...
        dev->peer = xstrdup(peer);
        netdev_change_seq_changed(dev_);
        netdev_dpdk_set_peer(dev_, peer);
    }
    ovs_mutex_unlock(&dev->mutex);
...
void netdev_vport_dpdk_patch_register(void);

void
netdev_vport_dpdk_patch_register(void)
{
    static const struct vport_class patch_class =
        { NULL,
            { "dpdkpatch", VPORT_FUNCTIONS(get_dpdkpatch_config,
                                       set_dpdkpatch_config,
                                       NULL,
                                       NULL, NULL, NULL, NULL) }};
    netdev_register_provider(&patch_class.netdev_class);
}
